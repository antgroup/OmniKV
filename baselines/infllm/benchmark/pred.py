...
sys.path.append("baselines/infllm/")
...
def parse_args_with_path(config_path):
    conf = OmegaConf.load(config_path)
    conf.model.model_center = False
    conf.rank = 0
    conf.world_size = 1
    conf.verbose = False
    if not hasattr(conf.model, "tokenizer_path"):
...
def get_model_and_tokenizer(config, quant_config=None):
    tokenizer = AutoTokenizer.from_pretrained(config.tokenizer_path)
    if config.model_center:
        raise ValueError("这里的原始代码被移除")
    else:
...
def get_pred(
    ...
    for json_obj in tqdm(data):
        ...
        #  Warning 在这个地方为了对齐，都不使用template
        ...
        if truncation is None:
            ...
            if truncation == "suffix":
                ...
                if length > max_length - max_gen:
                    ...
                    prompt = tokenizer.decode(
                        tokenized_prompt[:half_len].tolist()
                        + tokenized_prompt[-half_len:].tolist()
                    )
                    tokenized_prompt = tokenizer(
                        prompt,
...
if __name__ == "__main__":
    ...
    dataset2prompt = json.load(
        open("baselines/infllm/benchmark/config/dataset2prompt.json", "r")
    )
    dataset2maxlen = json.load(
        open("baselines/infllm/benchmark/config/dataset2maxlen.json", "r")
    )
    ...
    for dataset in datasets:
        ...
        if dataset in {
            ...
            path = "/input/fangshuai.fs/huggingface/repo/InfiniteBench"
            ...
            data = load_dataset(
                ...
                f"benchmark/long_bench/data/{dataset}.jsonl"
            )
