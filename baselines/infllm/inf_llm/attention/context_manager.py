...
class ContextManager:
    ...
    def get_block_k(self, k, score):
        ...
        ret = torch.gather(k, -2, score_topk[:, :, :, None].expand(self.num_units, self.unit_size, self.repr_topk,
                                                                   self.dim_head))
        return ret
    ...
    def from_group_kv(self, tensor):
        ...
        tensor = tensor.expand((self.num_units, self.unit_size_kv, num_group, length, dim_head)).reshape(
    ...
    def init(
        ...
        self.local_k = torch.empty((self.num_units, self.unit_size_kv, 0, dim_head), dtype=local_k.dtype,
                                   device=local_k.device)
        self.local_v = torch.empty((self.num_units, self.unit_size_kv, 0, dim_head), dtype=local_v.dtype,
        ...
        self.global_remainder_local_score = torch.empty((self.num_units, self.unit_size, 0), dtype=global_k.dtype,
                                                        device=global_k.device)

        self.init_k = torch.empty((self.num_units, self.unit_size_kv, 0, dim_head), dtype=global_k.dtype,
                                  device=global_k.device)
        self.init_v = torch.empty((self.num_units, self.unit_size_kv, 0, dim_head), dtype=global_k.dtype,
                                  device=global_k.device)
        self.init_exc = False
    ...
    def _append(
        ...
        with torch.cuda.stream(GLOBAL_STREAM):
            ...
            global_h_q = global_q
            global_h_k, global_h_v, global_sliding_window, global_block_map, global_block_num = self.get_global_hidden_and_mask(
                local_h_q.size(-2), block_topk)
    ...
    def get_batched_topk(self, global_q):
        ...
        block_k = block_k.reshape(self.num_units, self.num_global_block, self.unit_size, self.dim_head).permute(0, 2, 1,
                                                                                                                3).contiguous()
    ...
    def append_global(
        ...
        self.global_remainder_local_score[:, :, global_remainder_ed - local_score.size(-1):global_remainder_ed].add_(
            local_score)
            ...
            if use_chunk_topk and calc_cur_list[self._topk_calc_cur + 1] < ed:
                ...
                with torch.cuda.stream(GLOBAL_STREAM):
                    self._cached_topk = self.get_batched_topk(
                        global_q[:, :, calc_cur_list[self._topk_calc_cur]: calc_cur_list[self._topk_calc_cur + 1], :])
                self._topk_cur = 0
