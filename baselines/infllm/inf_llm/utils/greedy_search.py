    ...
    def _decode(self, input_ids, max_length=100, extra_end_token_ids=[], chunk_size: int = 4096, output=False):
        ...
        for i in range(max_length + 1):
            if i == 0:
                ...
                logits, past_key_values = out.logits, out.past_key_values
                logger.info(f"--- infllm prefill time {time.time() - last_t}s")
                last_t = time.time()
            else:
                out = self.model(
                    input_ids=input_ids[:, -1:],
                ...
                logits, past_key_values = out.logits, out.past_key_values
                logger.info(f"--- infllm decoding time {time.time() - last_t}s")
                last_t = time.time()
            ...
            attention_mask = torch.cat(
                (
                    attention_mask,
                    torch.ones((attention_mask.size(0), 1), dtype=torch.int, device=attention_mask.device)),
                dim=-1
            )
            if output:
