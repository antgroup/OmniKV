...
class TpPartBaseModel:
    ...
    def _prefill(
        ...
        if b_ready_cache_len is not None:
            ...
            infer_state.b_ready_cache_len = torch.zeros_like(
                b_seq_len, dtype=b_seq_len.dtype, device=b_seq_len.device
            )
    ...
    def _decode(
        ...
        alloc_mem = (
            None
            if self.graph is not None
            else self.mem_manager.alloc_contiguous(batch_size)
        )
        if alloc_mem is not None:
            infer_state.mem_is_contiguous = True
            infer_state.mem_index = alloc_mem[0]
        ...
        if self.graph is not None and self.graph.can_run(batch_size, max_len_in_batch):
            if self.graph.need_capture(batch_size):
                ...
                predict_logics = self.graph.capture_decode(
                    self._token_forward, input_ids, infer_state
                )
            else:
    ...
    def _context_forward(self, input_ids, infer_state: InferStateInfo):
        ...
        predict_logics = self.post_infer.token_forward(
            input_embs, infer_state, self.pre_post_weight
        )
