...
class MemoryManager:
    def __init__(
        ...
        self.shared_can_use_token_num = SharedInt(
            f"{str(nccl_port)}_mem_manger_can_use_token_num"
        )

        self.shared_can_use_token_num.set_value(self.can_use_mem_size)
        self._init_buffers(  # 这里应该是创建了全部的kv cache
            self.size,
            dtype,
            head_num,
            head_dim,
    ...
    def _init_buffers(self, size, dtype, head_num, head_dim, layer_num):
        # if os.environ.get("DO_OFFLOAD", False):
        #     self.kv_buffer = [
        #         torch.empty(
        #             (size, 2 * head_num, head_dim),
        #             dtype=dtype,
        #             device=(
        #                 "cuda" if _ < do_sel_layers[0] or _ in do_sel_layers else "cpu"
        #             ),
        #         )
