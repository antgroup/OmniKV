...
class LlamaPostLayerInfer(PostLayerInferTpl):
    ...
    def _slice_get_last_input(self, input_embdings, infer_state: LlamaInferStateInfo):
        ...
        if (
            not infer_state.is_splitfuse
            and infer_state.is_prefill
            and not infer_state.return_all_prompt_logics
        ):
            batch_size = infer_state.batch_size
            last_input = self.alloc_tensor(
                (batch_size, self.embed_dim_), dtype=input_embdings.dtype
        ...
        if (
            not infer_state.is_splitfuse
            and infer_state.is_prefill
            and infer_state.return_all_prompt_logics
        ):
    ...
    def token_forward(
        ...
        if self.world_size_ == 1:
            ...
            split_indexes = np.linspace(
                0, self.vocab_size_, self.world_size_ + 1, dtype=np.int64
            )
            dist.all_gather(
                [
        ...
        ans_logics = self.alloc_tensor(
            (token_num, self.vocab_size_), dtype=torch.float32, is_graph_out=True
        )
