...
from lightllm.models.llama_omnikv.layer_weights.transformer_layer_weight import (
    LlamaTransformerLayerWeight,
)
from lightllm.models.llama_omnikv.triton_kernel.context_flashattention_nopad import (
    context_attention_fwd,
    context_attention_fwd_no_prompt_cache,
)
from lightllm.models.llama_omnikv.triton_kernel.token_attention_nopad_att1 import (
    token_att_fwd,
    token_att_fwd_int8k,
)
from lightllm.models.llama_omnikv.triton_kernel.token_attention_nopad_softmax import (
    token_softmax_fwd,
)
from lightllm.models.llama_omnikv.triton_kernel.token_attention_nopad_reduceV import (
    token_att_fwd2,
    token_att_fwd2_int8v,
)
from lightllm.models.llama_omnikv.triton_kernel.rmsnorm import rmsnorm_forward
from lightllm.models.llama_omnikv.triton_kernel.rotary_emb import rotary_emb_fwd
from lightllm.models.llama_omnikv.triton_kernel.silu_and_mul import silu_and_mul_fwd

from lightllm.models.llama_omnikv.infer_struct import LlamaInferStateInfo
from lightllm.models.llama_omnikv.splitfuse_infer_struct import SplitFuseInferStateInfo
from lightllm.common.basemodel.triton_kernel.destindex_copy_kv import (
    destindex_copy_kv,
    destindex_copy_quantize_kv,
)
from lightllm.common.basemodel import TransformerLayerInferTpl
from lightllm.models.llama_omnikv.triton_kernel.splitfuse_context_flashattention_nopad import (
...
class LlamaTransformerLayerInfer(TransformerLayerInferTpl):
    ...
    def __init__(self, layer_num, tp_rank, world_size, network_config, mode=[]):
        ...
        self.is_70b = network_config["num_attention_heads"] > 32
        if network_config["num_attention_heads"] > 32:
            global do_sel_layers
            do_sel_layers = [4, 19, 41]
    ...
    def _att_norm(
        ...
        rmsnorm_forward(
            input, weight=layer_weight.att_norm_weight_, eps=self.eps_, out=out
        )
        return out
    ...
    def _ffn_norm(
        ...
        out = self.alloc_tensor(input.shape, input.dtype)
        rmsnorm_forward(
            input, weight=layer_weight.ffn_norm_weight_, eps=self.eps_, out=out
        )
        return out
    ...
    def _get_qkv(
        ...
        q = self.alloc_tensor(
            (input.size(0), layer_weight.q_weight_.size(1)), dtype=input.dtype
        )
    ...
    def _context_attention_kernel(
        ...
        if infer_state.use_dynamic_prompt_cache:
            ...
            infer_state.prefill_layer_idx += 1
            # assert infer_state.prefill_layer_idx not in infer_state.mem_manager.cache_cpu_kv
            if (
                infer_state.prefill_layer_idx not in do_sel_layers
                and infer_state.prefill_layer_idx > do_sel_layers[0]
            ):
                print(f"do offload now {infer_state.prefill_layer_idx}")
                infer_state.mem_manager.cache_cpu_kv[infer_state.prefill_layer_idx] = kv.to(
                    device="cpu", non_blocking=(infer_state.prefill_layer_idx != 31)
                )
    ...
    def _get_o(
        ...
        o_tensor = self.alloc_tensor(
            (input.size(0), layer_weight.o_weight_.size(1)), input.dtype
        )
    ...
    def _ffn(
        ...
        up_gate_out = self.alloc_tensor(
            (input.size(0), layer_weight.gate_up_proj.size(1)), input.dtype
        )
        torch.mm(input, layer_weight.gate_up_proj, out=up_gate_out)
        ffn1_out = silu_and_mul_fwd(up_gate_out)
        input = None
        up_gate_out = None
        ffn2_out = self.alloc_tensor(
            (ffn1_out.size(0), layer_weight.down_proj.size(1)), ffn1_out.dtype
        )
    ...
    def _copy_kv_to_mem_cache_ppl_int8kv(self, buffer, mem_index, mem_manager):
        from lightllm.models.llama_omnikv.triton_kernel.ppl_quant_copy_kv import (
            destindex_copy_quantize_kv,
        )
    ...
    def _copy_kv_to_mem_cache_ppl_int4kv(self, buffer, mem_index, mem_manager):
        from lightllm.models.llama_omnikv.triton_kernel.ppl_int4kv_copy_kv import (
    ...
    def _token_decode_attention_normal(
        ...
        o_tensor = self.alloc_tensor(q.shape, q.dtype) if out is None else out
        from lightllm.models.llama_omnikv.triton_kernel.token_attention_softmax_and_reducev import (
    ...
    def _token_decode_attention_flashdecoding(
        ...
        from lightllm.models.llama_omnikv.triton_kernel.flash_decoding import (
        ...
        # +++ start hack +++
        infer_state.now_layer_idx += 1
        do_select = infer_state.now_layer_idx in do_sel_layers
        # assert infer_state.now_layer_idx < 32
    ...
    def _token_decode_attention_ppl_fp16_flashdecoding(
        ...
        from lightllm.models.llama_omnikv.triton_kernel.ppl_fp16_flash_decoding import (
            token_decode_attention_flash_decoding,
        )
    ...
    def _token_decode_attention_ppl_int8kv_flashdecoding(
        ...
        from lightllm.models.llama_omnikv.triton_kernel.ppl_int8kv_flash_decoding import (
            token_decode_attention_flash_decoding,
        )
    ...
    def _token_decode_attention_ppl_int4kv_flashdecoding(
        ...
        from lightllm.models.llama_omnikv.triton_kernel.ppl_int4kv_flash_decoding import (
            token_decode_attention_flash_decoding,
        )
