...
def _fwd_kernel_flash_decode_stage1_with_score(
    ...
    for start_n in range(0, block_n_size, 1):
        ...
        # current_max_attn_score = tl.load(Attn_Score + cur_batch * stride_asbs + cur_head * stri + offs_n_new * stride_asl, mask=offs_n_new < cur_batch_end_index, other=float('-inf'))
        # current_max_attn_score = tl.maximum(current_max_attn_score, att_value)
        tl.store(
            Attn_Score + cur_batch * stride_asbs + cur_head * stride_ash + offs_n_new,
            att_value,
            mask=offs_n_new < cur_batch_end_index,
        )
        att_value *= sm_scale
